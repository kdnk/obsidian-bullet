...
Obsidianの`.cm-indent::before`は明示的なinline insetではなく、各indent spanのstatic positionとmode別のnative marginを基準に`border-inline-end`で1px線を描く。
ここへ`inset-inline-end`を追加するとstatic positionが失われ、線が行の反対側へ移動するため使わない。
marginを単純な`-1px`へ置き換える方法も、Live Preview既定の`--indentation-guide-editing-indent: 0.85em`を失わせるため使わない。
hover markerが付いたpseudo-elementへ3px borderと2px radiusを適用し、mode別のnative marginから1pxだけ差し引く。
...
これにより、static positionとnative mode offsetを維持したままpseudo-element全体をinline start側へ1px戻し、3px borderを元の1px線の両側へ均等に広げる。
...
CSS契約テストでは、native hover ruleが3px borderと`border-radius: 2px`を持ち、Live PreviewとSource modeの各ruleが対応するnative margin変数から1pxを差し引くことを確認する。
static positionを上書きするinline insetや、native marginを失わせる固定値`margin-inline-start: -1px`は認めない。
