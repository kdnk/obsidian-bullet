...
- テストについて
    ...
    - フルテストは `vault/test.md` をfixtureとして上書きします。手動検証用の内容がある場合は、フルテスト前にvault外へbackupしてください。テストcommandの終了直後はtest rendererの遅延終了処理がfixtureを再保存する場合があるため、`vault=vault` のrendererが終了したことを確認してからrestoreしてください。rendererが30秒を超えて残る場合があるため、固定timeout到達を終了確認の代わりにしないでください。restore時点でrendererが残っていた場合は、実際の終了後にもfile sizeまたはhashを再確認してください。restore後は少し待ってfile sizeまたはhashがbackupと一致することを再確認してから実Obsidianの検証を続けてください。Obsidianのfile historyだけをbackupとして使わないでください。
...
- 縦線ガイドについて
    ...
    - `.cm-indent` は `min-width` と `inline-block` も適用するため、spacing span へ付与するだけでは nesting の横位置が変わります。plugin marker に限定して `min-width: 0` と `display: inline` を適用し、元の spacing 幅を維持してください。persistent guide の通常表示では、線の offset・太さを上書きしないでください。通常色は個別markerへ設定せず、enhanced vertical lines設定が有効なview全体の`--indentation-guide-color`へ`color-mix(in oklch, var(--text-normal) 20%, transparent)`を指定し、native segmentと同じ色へ揃えてください。
    - クリック可能な guide の hover feedback は、その行の fragment や同じ X 座標の全 guide ではなく、クリック時の exact raw-prefix resolver が同じ実リスト祖先へ対応付ける表示中の native segment 全体へ適用してください。toggle folding action が有効なときだけ plugin marker を付け、既存の native `.cm-indent::before` へ Obsidian の `--indentation-guide-width-active` と `--indentation-guide-color-active` を適用してください。enhanced表示では線を中心固定の3pxへ太くし、同じ論理groupの先頭segmentの上端と末尾segmentの下端だけを2pxで丸め、中間segmentを角丸にしないでください。inner guideとouter guideのendpoint markerはCodeMirrorのDOM更新後に現在の`:hover`要素から付け直し、pointer leave・action無効化・ViewPlugin destroy時に通常のhover markerと一緒に除去してください。固定色、独自geometry、overlay、座標cache、背景highlight、通常線の幅や位置の変更は追加しないでください。
...
- native chevron のスクロール保持について
    ...
    - snapshotは、次の`foldEffect`、`unfoldEffect`、またはfolded rangeの実内容が変わるnative transactionへtransaction extenderで追加してください。先にselection-only transactionが入る場合は同じevent turnの後続stateへ引き継ぎ、fold状態を変えるtransactionと同じtransactionで一度だけ消費してください。
    - native fold transactionを独自handlerで置き換えず、fold後の手動`scrollTop`復元、遅延したscroll補正、browserの`overflow-anchor`への切替を追加しないでください。実Obsidianではlistとheadingのfoldとunfoldをnative pointer sequenceで測り、操作行、上側の表示行、`scrollTop`の各spanが0pxで、fold状態が一度だけ反転することを確認してください。
- モバイルの右端折りたたみコントロールについて
    ...
    - mobile native chevronのscroll保持対象selectorは、右端へ配置するすべてのcontrol種別を含めてください。現在は`.HyperMD-list-line`と`.HyperMD-header`の両方が対象です。control種別を追加するときはCSSだけでなくcapture phaseの`pointerdown`と`click`の対象も同時に広げ、各targetで下端余白の復元と`NativeFoldScrollState.prepare()`が実行されるunit testを追加してください。
